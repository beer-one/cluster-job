...
type PerNodeGroupStrategy struct {
	...
	IgnoreNull bool `json:"ignoreNull"`
}
...
func (cj *ClusterJob) SavePendingStatus(nodeGroups []NodeGroupStatus) {
	cj.Status.Phase = "Pending"
	cj.Status.CurrentIndex = -1
	cj.Status.CurrentGroup = ""
	cj.Status.WaitGroups = len(nodeGroups)
	cj.Status.NodeGroups = nodeGroups
}

func (cj *ClusterJob) SaveRunningStatus(currentIndex int, currentGroup string, waitGroups int) {
	cj.Status.Phase = "Running"
	cj.Status.CurrentIndex = currentIndex
	cj.Status.CurrentGroup = currentGroup
	cj.Status.WaitGroups = waitGroups
}

func (cj *ClusterJob) SaveFailedStatus() {
	cj.Status.Phase = "Failed"
}

func (cj *ClusterJob) SaveCompletedStatus() {
